...
export const metadata: Metadata = {
  title: "Üye Ol",
  description:
    "Miss Bella'ya üye olarak siparişlerinizi takip edin, favorilerinizi cihazlar arasında senkronlayın.",
        ...
        <Container className="mx-auto max-w-md">
          <div className="text-center">
            ...
            <h1 className="mt-3 font-display text-[40px] leading-[1.05] text-ink-900 md:text-[52px]">
              Miss <span className="font-italic-display text-gradient-fuchsia">Bella</span>&apos;ya Üye Ol
            </h1>
            <p className="mx-auto mt-4 max-w-sm text-sm text-ink-700">
              Siparişlerinizi takip edin, favorilerinizi cihazlar arasında
              senkronlayın ve hızlı ödeme yapın.
            </p>
          </div>

          <div className="mt-10 border border-ink-900/10 bg-white p-7 shadow-card md:p-8">
            <RegisterForm redirectTo={sanitizeRedirect(params.redirect)} />

            <p className="mt-6 text-center text-sm text-ink-700">
              Zaten üyeyim?{" "}
              <Link
                href={`/giris${params.redirect ? `?redirect=${encodeURIComponent(params.redirect)}` : ""}`}
